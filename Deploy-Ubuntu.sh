...
# =============================================================
#  MULTI-RELAY XRAY HELPERS
# =============================================================
readonly XRAY_CFG="/usr/local/etc/xray/config.json"
readonly XHTTP_STATE_DIR="/etc/xhttp-installer"
readonly RELAY_REGISTRY_DIR="${XHTTP_STATE_DIR}/relays"
readonly RELAY_PORT_BASE=2096

ensure_jq_for_xray() {
  if command -v jq &>/dev/null; then return 0; fi
  info "Installing jq (required for multi-relay config)..."
  DEBIAN_FRONTEND=noninteractive apt-get update -qq 2>/dev/null || true
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq jq 2>/dev/null || {
    fail "jq is required. Install manually: apt install -y jq"
    return 1
  }
  ok "jq installed"
}

# User tag → canonical inbound tag (relay-<name>)
relay_full_tag() {
  local raw="${1// /}"
  raw="${raw#relay-}"
  echo "relay-${raw}"
...
xray_find_free_port() {
  local cfg="${1:-$XRAY_CFG}" port=$RELAY_PORT_BASE used=""
  ensure_jq_for_xray || return 1
  if [[ -f "$cfg" ]]; then
    used=$(jq -r '.inbounds[]? | select(.port != null) | .port' "$cfg" 2>/dev/null | sort -nu || true)
  fi
...
# Echo: none | tag:<name> | port:<num>
xray_inbound_conflict() {
  local full_tag="$1" port="$2" cfg="${3:-$XRAY_CFG}"
  ensure_jq_for_xray || return 1
  [[ -f "$cfg" ]] || { echo "none"; return 0; }
  if jq -e --arg t "$full_tag" '.inbounds[]? | select(.tag == $t)' "$cfg" &>/dev/null; then
    echo "tag:${full_tag}"
    return 0
  fi
  if jq -e --argjson p "$port" '.inbounds[]? | select(.port == $p)' "$cfg" &>/dev/null; then
    echo "port:${port}"
    return 0
  fi
  echo "none"
...
# backup → write candidate → xray -test → rollback on fail → restart on success
xray_apply_config() {
  local candidate="$1" cfg="${2:-$XRAY_CFG}"
  local backup="" test_out

  if [[ ! -f "$candidate" ]]; then
    fail "Internal error: missing candidate config ${candidate}"
    return 1
  fi

  if [[ -f "$cfg" ]]; then
    backup=$(xray_config_backup "$cfg")
    [[ -n "$backup" ]] && info "Config backup: ${backup}"
  fi
  ...
  fail "xray failed to start after config apply"
  if [[ -n "$backup" && -f "$backup" ]]; then
    warn "Rolling back to backup: ${backup}"
    cp "$backup" "$cfg"
    systemctl restart xray 2>/dev/null || true
  fi
...
xray_build_inbound_json() {
  # Args: tag listen port uuid path domain [platform]
  local tag="$1" listen="$2" port="$3" uuid="$4" path="$5" domain="$6"
  local platform="${7:-vercel}"
  local cert="${SSL_CERT:-/etc/ssl/xhttp/${domain}/fullchain.pem}"
  local key="${SSL_KEY:-/etc/ssl/xhttp/${domain}/privkey.pem}"
  local padding="100-1000"
  local xhttp_extra='{}'

  if [[ "$platform" == "netlify" ]]; then
    padding="${XPADDING:-10-50}"
    xhttp_extra=$(jq -n \
      --arg k "${XPADDING_KEY:-}" \
      --arg h "${XPADDING_HEADER:-}" \
      --arg sc "${SC_MAX_POST_BYTES:-1000000}" \
      '{xPaddingObfsMode: true, xPaddingKey: $k, xPaddingHeader: $h, scMaxEachPostBytes: $sc}')
  fi
...
xray_merge_inbound() {
  # Merge or replace one inbound; writes merged JSON to $1 (output file)
  local out_file="$1" inbound_json="$2" full_tag="$3" replace="${4:-false}" port="$5"
  local cfg="$XRAY_CFG" skeleton
  ...
  if [[ ! -f "$cfg" ]]; then
    echo "$skeleton" | jq --argjson ib "$inbound_json" '.inbounds += [$ib]' > "$out_file"
    return 0
  fi

  if [[ "$replace" == "true" ]]; then
    jq --arg t "$full_tag" --argjson p "$port" --argjson ib "$inbound_json" \
      '.inbounds |= (map(select(.tag != $t and .port != $p)) + [$ib])' "$cfg" > "$out_file"
  else
    jq --argjson ib "$inbound_json" '.inbounds += [$ib]' "$cfg" > "$out_file"
  fi
}

xray_test_inbound() {
  local listen="$1" port="$2" path="$3"
  local host="127.0.0.1"
  [[ "$listen" == "0.0.0.0" || "$listen" == "::" ]] && host="127.0.0.1"
  local http_code
  http_code=$(curl -sk --max-time 5 \
    "https://${host}:${port}${path}" \
    -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
  if echo "$http_code" | grep -qE "^(4[0-9]{2}|200)$"; then
    ok "Inbound local test: HTTP ${http_code} on ${host}:${port}${path}"
    return 0
  fi
  warn "Inbound local test: HTTP ${http_code} on ${host}:${port}${path} (may be normal for XHTTP)"
  return 0
}

relay_build_client_link() {
  local VERCEL_HOST ENCODED_PATH EXTRA_JSON ENCODED_EXTRA LINK_TAG
  ...
  LINK_TAG="XHTTP-${CFG_PLATFORM}-${CFG_RELAY_TAG:-relay}"
  ...
  echo "vless://${INBOUND_UUID}@${VERCEL_HOST}:443?encryption=none&security=tls&sni=${VERCEL_HOST}&fp=chrome&alpn=h2%2Chttp%2F1.1&insecure=0&allowInsecure=0&type=xhttp&host=${VERCEL_HOST}&path=${ENCODED_PATH}&mode=auto&extra=${ENCODED_EXTRA}#${LINK_TAG}"
}

relay_save_registry() {
  local tag_file="${RELAY_REGISTRY_DIR}/${CFG_RELAY_TAG}.env"
  mkdir -p "$RELAY_REGISTRY_DIR"
  chmod 700 "$XHTTP_STATE_DIR" 2>/dev/null || true
  local client_link
  client_link=$(relay_build_client_link)
  cat > "$tag_file" <<REG
CFG_RELAY_TAG="${CFG_RELAY_TAG}"
CFG_RELAY_FULL_TAG="${CFG_RELAY_FULL_TAG}"
...
CFG_LISTEN_ADDR="${CFG_LISTEN_ADDR}"
...
ssl_cert_valid_on_disk() {
  local cert="${1:-${SSL_CERT:-}}"
  [[ -n "$cert" && -f "$cert" ]] || return 1
  openssl x509 -in "$cert" -noout -checkend 86400 2>/dev/null
}

ssl_discover_existing_cert() {
  local domain="${1:-${CFG_DOMAIN:-}}"
  [[ -n "$domain" ]] || return 1
  SSL_DIR="/etc/ssl/xhttp/${domain}"
  SSL_CERT="${SSL_DIR}/fullchain.pem"
  SSL_KEY="${SSL_DIR}/privkey.pem"
  ssl_cert_valid_on_disk "$SSL_CERT"
}

relay_list_cmd() {
  ensure_jq_for_xray || exit 1
  if [[ ! -f "$XRAY_CFG" ]]; then
    info "No Xray config at ${XRAY_CFG}"
    exit 0
  fi
  local count
  count=$(jq '.inbounds | length' "$XRAY_CFG" 2>/dev/null || echo 0)
  if [[ "$count" -eq 0 ]]; then
    info "No inbounds in ${XRAY_CFG}"
    exit 0
  fi
  echo ""
  echo -e "  ${C_CYAN}── Xray inbounds (${count}) ──${C_RESET}"
  echo ""
  jq -r '.inbounds[] |
    "  tag:   \(.tag // "?")\n  port:  \(.port // "?")\n  path:  \(.streamSettings.xhttpSettings.path // "?")\n  uuid:  \(.settings.clients[0].id // "?")\n  listen:\(.listen // "?")\n  ---"' "$XRAY_CFG"
  echo ""
  if [[ -d "$RELAY_REGISTRY_DIR" ]]; then
    echo -e "  ${C_CYAN}── Saved relay registry ──${C_RESET}"
    for f in "$RELAY_REGISTRY_DIR"/*.env; do
      [[ -f "$f" ]] || continue
      # shellcheck source=/dev/null
      source "$f"
      echo -e "  ${C_WHITE}${CFG_RELAY_TAG:-?}${C_RESET} → port ${CFG_INBOUND_PORT:-?} ${VERCEL_URL:-}"
    done
    echo ""
  fi
  exit 0
...
relay_remove_cmd() {
  local user_tag="$1"
  local full_tag conflict backup merged
  [[ $EUID -eq 0 ]] || { fail "Run as root: sudo bash Deploy-Ubuntu.sh --remove <tag>"; exit 1; }
  ensure_jq_for_xray || exit 1
  full_tag=$(relay_full_tag "$user_tag")
  [[ -f "$XRAY_CFG" ]] || { fail "No config at ${XRAY_CFG}"; exit 1; }

  if ! jq -e --arg t "$full_tag" '.inbounds[] | select(.tag == $t)' "$XRAY_CFG" &>/dev/null; then
    fail "No inbound with tag '${full_tag}'"
    info "Use --list to see existing tags"
    exit 1
  fi

  echo -e "  ${C_YELLOW}Remove inbound '${full_tag}'? Other inbounds are kept.${C_RESET}"
  confirm "Confirm removal" || { info "Cancelled."; exit 0; }
  ...
  rm -f "${RELAY_REGISTRY_DIR}/${user_tag}.env" "${RELAY_REGISTRY_DIR}/${user_tag#relay-}.env" 2>/dev/null || true
  ok "Removed inbound '${full_tag}'"
  exit 0
}
...
cli_early_dispatch() {
  case "${1:-}" in
    --list)  relay_list_cmd ;;
    --remove)
      [[ -n "${2:-}" ]] || { fail "Usage: sudo bash Deploy-Ubuntu.sh --remove <relay-tag>"; exit 1; }
      relay_remove_cmd "$2"
      ;;
  esac
}
...
# =============================================================
_load_server_identity_from_disk() {
  if [[ -f "${XHTTP_STATE_DIR}/info.env" ]]; then
    # shellcheck source=/dev/null
    source "${XHTTP_STATE_DIR}/info.env"
  fi
  if [[ -z "${CFG_DOMAIN:-}" && -d /etc/ssl/xhttp ]]; then
    local cert_path d
    for cert_path in /etc/ssl/xhttp/*/fullchain.pem; do
      [[ -f "$cert_path" ]] || continue
      if ssl_cert_valid_on_disk "$cert_path"; then
        d=$(basename "$(dirname "$cert_path")")
        CFG_DOMAIN="$d"
        SSL_DIR="/etc/ssl/xhttp/${d}"
        SSL_CERT="${SSL_DIR}/fullchain.pem"
        SSL_KEY="${SSL_DIR}/privkey.pem"
        break
      fi
...
phase3_collect_input() {
  ...
  echo -e "  ${C_GRAY}Add one relay per run. Existing inbounds are kept. Use --list to view all.${C_RESET}\n"

  _load_server_identity_from_disk
  SKIP_SSL_ISSUE=false
  if ssl_discover_existing_cert "${CFG_DOMAIN:-}"; then
    SKIP_SSL_ISSUE=true
    ok "Valid SSL cert on disk for ${CFG_DOMAIN} — skipping domain/email prompts"
  fi

  # ── SSL / Domain (server-wide, shared across relays) ─────
  if [[ "$SKIP_SSL_ISSUE" != "true" ]]; then
    echo -e "\n  ${C_CYAN}[ SSL & Domain (server) ]${C_RESET}"
    ...
    [[ -z "${CFG_EMAIL:-}" ]] && CFG_EMAIL=$(grep -E '^CFG_EMAIL=' "${XHTTP_STATE_DIR}/info.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || true)
  fi

  # ── This relay (unique tag, port, path) ───────────────────
  echo -e "\n  ${C_CYAN}[ Relay identity ]${C_RESET}"
  while true; do
    CFG_RELAY_TAG=$(read_required "Relay name (short tag, e.g. apiA)")
    CFG_RELAY_TAG="${CFG_RELAY_TAG#relay-}"
    CFG_RELAY_TAG="${CFG_RELAY_TAG// /}"
    if [[ ! "$CFG_RELAY_TAG" =~ ^[a-zA-Z0-9][a-zA-Z0-9_-]*$ ]]; then
      fail "Use letters, numbers, underscore, hyphen only."
      continue
    fi
    CFG_RELAY_FULL_TAG=$(relay_full_tag "$CFG_RELAY_TAG")
    local pre_conflict
    pre_conflict=$(xray_inbound_conflict "$CFG_RELAY_FULL_TAG" "0" 2>/dev/null || echo "none")
    if [[ "$pre_conflict" == tag:* ]]; then
      fail "Tag '${CFG_RELAY_FULL_TAG}' already exists. Pick another name or use --remove."
      continue
    fi
    break
  done
  ok "Inbound tag will be: ${CFG_RELAY_FULL_TAG}"

  ensure_jq_for_xray 2>/dev/null || true
  local suggested_port
  suggested_port=$(xray_find_free_port 2>/dev/null || echo "$RELAY_PORT_BASE")

  echo -e "\n  ${C_CYAN}[ Inbound & paths ]${C_RESET}"
  CFG_INBOUND_PORT=$(read_default "Internal inbound port on server (XHTTP)" "$suggested_port")
  CFG_RELAY_PATH=$(read_default   "RELAY_PATH  (inbound path, e.g. /apiA)" "/api")
  CFG_PUBLIC_PATH=$(read_default  "PUBLIC_RELAY_PATH (CDN-side path)" "/api")
  [[ "${CFG_RELAY_PATH:0:1}" != "/" ]] && CFG_RELAY_PATH="/$CFG_RELAY_PATH"
  [[ "${CFG_PUBLIC_PATH:0:1}" != "/" ]] && CFG_PUBLIC_PATH="/$CFG_PUBLIC_PATH"

  INBOUND_UUID=""
  local uuid_in
  uuid_in=$(read_default "Client UUID (leave empty to auto-generate)" "")
  [[ -n "${uuid_in// }" ]] && INBOUND_UUID=$(echo "$uuid_in" | tr '[:upper:]' '[:lower:]')

  echo -e "\n  ${C_CYAN}[ Listen address ]${C_RESET}"
  echo -e "  ${C_GRAY}Public = Vercel/Netlify reach your server IP on this port.${C_RESET}"
  echo -e "  ${C_GRAY}Local  = 127.0.0.1 only (use with SSH tunnel).${C_RESET}"
  local listen_choice listen_default="1"
  if [[ -f "$XRAY_CFG" ]] && jq -e '.inbounds[]? | select(.listen == "127.0.0.1")' "$XRAY_CFG" &>/dev/null; then
    listen_default="2"
  fi
  listen_choice=$(read_default "Listen mode [1=public 0.0.0.0, 2=local 127.0.0.1]" "$listen_default")
  case "$listen_choice" in
    2|local|127*) CFG_LISTEN_ADDR="127.0.0.1" ;;
    *)            CFG_LISTEN_ADDR="0.0.0.0" ;;
  esac
  ok "Listen: ${CFG_LISTEN_ADDR}"
  ...
  echo -e "  ${C_WHITE}Server domain   :${C_RESET} $CFG_DOMAIN ${C_GRAY}(shared SSL)${C_RESET}"
  echo -e "  ${C_WHITE}Relay tag       :${C_RESET} ${CFG_RELAY_FULL_TAG}"
  echo -e "  ${C_WHITE}Listen          :${C_RESET} ${CFG_LISTEN_ADDR}"
  ...
  echo -e "  ${C_WHITE}TARGET_DOMAIN   :${C_RESET} https://${CFG_DOMAIN}:${CFG_INBOUND_PORT}"
  if [[ "$CFG_PLATFORM" == "vercel" ]]; then
...
# =============================================================
_phase4a_installcert_to_ssl_dir() {
  local ecc_flag="--ecc"
  local acme_cert_path="$HOME/.acme.sh/${CFG_DOMAIN}_ecc/${CFG_DOMAIN}.cer"
  [[ -f "$acme_cert_path" ]] || acme_cert_path="$HOME/.acme.sh/${CFG_DOMAIN}/${CFG_DOMAIN}.cer"
  [[ "$acme_cert_path" == *"${CFG_DOMAIN}/${CFG_DOMAIN}.cer" ]] && ecc_flag=""
  local ACME_CMD="${HOME}/.acme.sh/acme.sh"
  [[ -x "$ACME_CMD" ]] || ACME_CMD="$(command -v acme.sh 2>/dev/null || true)"
  [[ -x "$ACME_CMD" ]] || return 1
...
phase4a_ssl() {
  step "PHASE 4a — SSL for ${CFG_DOMAIN}"
  ...
  # ── Reuse valid on-disk cert: never touch port 80 or stop xray ──
  if ssl_cert_valid_on_disk "$SSL_CERT"; then
    local exp
    exp=$(openssl x509 -in "$SSL_CERT" -noout -enddate 2>/dev/null | cut -d= -f2- || true)
    ok "Valid SSL already installed — skipping acme.sh issue (xray stays running)"
  ...
  info "No valid cert in ${SSL_DIR} — will request from Let's Encrypt"

  # acme.sh may already have a valid cert — install to SSL_DIR without HTTP-01
  local _acme_cer_ec="${HOME}/.acme.sh/${CFG_DOMAIN}_ecc/${CFG_DOMAIN}.cer"
  local _acme_cer_rsa="${HOME}/.acme.sh/${CFG_DOMAIN}/${CFG_DOMAIN}.cer"
  for _acme_try in "$_acme_cer_ec" "$_acme_cer_rsa"; do
    if [[ -f "$_acme_try" ]] && openssl x509 -in "$_acme_try" -noout -checkend 86400 2>/dev/null; then
      info "Found valid acme.sh cert — installing to ${SSL_DIR} (no renewal, xray not stopped)"
      if _phase4a_installcert_to_ssl_dir; then
        ok "SSL certificate installed from acme.sh cache"
        return 0
      fi
      break
    fi
...
phase4b_configure_xray() {
  step "PHASE 4b — Adding Xray inbound (relay-${CFG_RELAY_TAG})"
  ...
  if [[ -z "${INBOUND_UUID:-}" ]]; then
    if INBOUND_UUID=$(xray uuid 2>/dev/null | tr -d '[:space:]'); then
      INBOUND_UUID=$(echo "$INBOUND_UUID" | tr '[:upper:]' '[:lower:]')
    else
    ...
    INBOUND_UUID=$(echo "$INBOUND_UUID" | tr '[:upper:]' '[:lower:]')
    ok "Using UUID: ${INBOUND_UUID}"
  fi

  # ── Platform-specific XHTTP tuning (must outlive this function) ──
  XPADDING=""
  XPADDING_KEY=""
  XPADDING_HEADER=""
  XPADDING_OBFS="false"
  ...
  # ── Conflict check (fail unless user confirms replace) ───
  local conflict replace=false
  conflict=$(xray_inbound_conflict "$CFG_RELAY_FULL_TAG" "$CFG_INBOUND_PORT")
  if [[ "$conflict" != "none" ]]; then
    fail "Inbound conflict: ${conflict}"
    if ! confirm "Replace the existing inbound? (other inbounds are kept)"; then
      fail "Aborted — no changes written"
      return 1
    fi
    replace=true
    warn "Will replace conflicting inbound: ${conflict}"
  fi

  # ── Merge inbound into config (never full-file overwrite) ─
  local inbound_json merged
  ...
  _ensure_xray_systemd_root
  xray_apply_config "$merged" || { rm -f "$merged"; return 1; }
  rm -f "$merged"

  if [[ "${CFG_LISTEN_ADDR}" == "0.0.0.0" ]]; then
    if ufw status 2>/dev/null | grep -qi "Status: active"; then
      ufw allow "${CFG_INBOUND_PORT}/tcp" 2>/dev/null || true
      ok "Firewall: allowed TCP ${CFG_INBOUND_PORT}"
    fi
  fi

  xray_test_inbound "$CFG_LISTEN_ADDR" "$CFG_INBOUND_PORT" "$CFG_RELAY_PATH"

  ok "Inbound tag: ${CFG_RELAY_FULL_TAG}  port: ${CFG_INBOUND_PORT}  path: ${CFG_RELAY_PATH}"
...
# =============================================================
phase6_summary() {
  local CLIENT_LINK TARGET_DOMAIN_VAL
  TARGET_DOMAIN_VAL="https://${CFG_DOMAIN}:${CFG_INBOUND_PORT}"
  CLIENT_LINK=$(relay_build_client_link)

  echo ""
  echo -e "${C_GREEN}  ╔══════════════════════════════════════════════════════════╗${C_RESET}"
  echo -e "${C_GREEN}  ║       Relay '${CFG_RELAY_FULL_TAG}' added  ✔              ║${C_RESET}"
  echo -e "${C_GREEN}  ╚══════════════════════════════════════════════════════════╝${C_RESET}"
  echo ""
  ...
  case "${E2E_STATUS:-UNKNOWN}" in
    PASS) echo -e "  ${C_WHITE}E2E test         :${C_RESET} ${C_GREEN}PASS${C_RESET} (${E2E_PING_AVG:-?} ms)" ;;
    FAIL) echo -e "  ${C_WHITE}E2E test         :${C_RESET} ${C_RED}FAIL${C_RESET} ${C_GRAY}(${E2E_DETAIL})${C_RESET}" ;;
    *)    echo -e "  ${C_WHITE}E2E test         :${C_RESET} ${C_YELLOW}not run${C_RESET}" ;;
  esac
  echo ""
  echo -e "  ${C_CYAN}── Client link (this relay only) ──${C_RESET}"
  echo ""
  echo -e "  ${C_YELLOW}${CLIENT_LINK}${C_RESET}"
  echo ""
  echo -e "  ${C_GRAY}All inbounds: sudo bash Deploy-Ubuntu.sh --list${C_RESET}"
  echo -e "  ${C_GRAY}Panel: xhttp  |  Log: ${LOG_FILE}${C_RESET}"
  echo ""
}
...
phase7_install_panel() {
  ...
  local VERCEL_HOST CLIENT_LINK
  VERCEL_HOST=$(echo "${VERCEL_URL:-}" | sed 's|https://||; s|/.*||')
  CLIENT_LINK=$(relay_build_client_link)
  relay_save_registry

  cat > "$STATE_FILE" <<STATE
# XHTTP Installer — last relay (see ${RELAY_REGISTRY_DIR}/ for all relays)
